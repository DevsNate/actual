...
| `src/index.ts` | MIXED export surface | EXTRACT-CONCEPT | Re-exports portable contracts and `stock-plan-bootstrap`; final canonical package should not expose stock representation from its root. |
...
| `src/plan.ts` | MIXED CANONICAL-CANDIDATE | EXTRACT-CONCEPT | Generic plan/entity/read/change-set contracts are useful; current `PlanEntity` remains representation-agnostic only structurally while callers store YNAB `be_*` kinds. |
| `src/plan-lifecycle.ts` | CANONICAL-CANDIDATE | PROMOTE-CANDIDATE | Atomic rename/delete command contracts and replay result shape. |
| `src/stock-plan-bootstrap.ts` | YNAB-COMPATIBILITY | KEEP-LAB | Admitted PLAN-001 stock `be_*` bootstrap defaults, system categories/payees, targets, identities, and field values. |
| `src/stock-plan-bootstrap.test.ts` | YNAB-COMPATIBILITY tests | KEEP-LAB | Executable evidence fixture for the stock bootstrap. Final YNAB adapter may derive tests from it, but it is not canonical domain logic. |
...
| `migrations/0001_semantic_foundation.sql` | CANONICAL-CANDIDATE | PROMOTE-CANDIDATE | Plans/memberships, plan/device knowledge, ordered change sets, entity changes, tombstones and receipts. Needs schema naming/domain review, not blind copy. |
...
| `src/canonical-plan-migration.ts` | BUILD/EMBED ADAPTER | REBUILD-FINAL | Same. |
...
| `src/plan-reader.ts` | CANONICAL-CANDIDATE | PROMOTE-CANDIDATE | Principal-authorized plan/budget-version read boundary; entity shape may change with final domain storage. |
| `src/plan-lifecycle-store.ts` | MIXED CANONICAL-CONCEPT + YNAB-COMPATIBILITY | EXTRACT-CONCEPT | Atomic lifecycle/replay/knowledge logic is reusable, but current rename directly rewrites `be_budget` payload and delete/catalog semantics include stock-derived details. |
...
| `src/semantic/plan-api.ts` | HTTP ADAPTER / TRANSITIONAL | REBUILD-FINAL | Native semantic read/create Express routes; delegates creation to application service but is coupled to current envelope/auth host. |
| `src/semantic/plan-api.test.ts` | adapter tests | KEEP-LAB | Current host contract proof. |
| `src/semantic/plan-lifecycle-api.ts` | HTTP ADAPTER / TRANSITIONAL | REBUILD-FINAL | Native rename/delete transport over lifecycle service. |
| `src/semantic/plan-lifecycle-api.test.ts` | adapter tests | KEEP-LAB | Current host contract proof. |
...
| `src/semantic/plan-creation-service.ts` | MIXED CANONICAL-CONCEPT + YNAB-COMPATIBILITY | EXTRACT-CONCEPT | Good shared application-service boundary, but directly invokes `buildStockPlanBootstrap` and returns stock-shaped IDs. Preserve orchestration, separate canonical plan creation from YNAB bootstrap projection. |
| `src/semantic/plan-lifecycle-service.ts` | CANONICAL-CANDIDATE application service | PROMOTE-CANDIDATE | Centralizes change IDs, digests and lifecycle writer calls outside HTTP. Review response shapes/stock-derived deletion semantics before promotion. |
...
| `packages/desktop-client/src/semantic-plans/api.ts` | ACTUAL-ADAPTER | KEEP-LAB | React-facing wrappers over Actual worker message bus; allocates idempotency IDs. |
| `packages/desktop-client/src/semantic-plans/api.test.ts` | ACTUAL-ADAPTER tests | KEEP-LAB | Worker bridge contract proof. |
| `packages/desktop-client/src/semantic-plans/semanticPlansSlice.ts` | ACTUAL-ADAPTER / TRANSITIONAL | KEEP-LAB | Redux catalog/plan state while legacy Actual budget state still exists. |
| `packages/desktop-client/src/semantic-plans/semanticPlansSlice.test.ts` | ACTUAL-ADAPTER tests | KEEP-LAB | Current client-state proof. |
...
| `packages/loot-core/src/server/semantic-plans/app.ts` | ACTUAL-ADAPTER | KEEP-LAB | Worker methods for catalog/read/create/rename/delete using retained server config/token/device identity. |
| `packages/loot-core/src/server/semantic-plans/http.ts` | ACTUAL-ADAPTER | KEEP-LAB | Actual platform fetch wrapper for `/semantic/v1`. |
| `packages/loot-core/src/server/semantic-plans/http.test.ts` | ACTUAL-ADAPTER tests | KEEP-LAB | Header/envelope/error translation tests. |
| `packages/loot-core/src/server/semantic-plans/identity.ts` | ACTUAL-ADAPTER / CANONICAL-CONCEPT | EXTRACT-CONCEPT | Durable client device identity is a useful sync concept, but implementation uses Actual async storage. |
| `packages/loot-core/src/server/semantic-plans/identity.test.ts` | ACTUAL-ADAPTER tests | KEEP-LAB | Current storage behavior. |
| `packages/loot-core/src/server/semantic-plans/types.ts` | ACTUAL-ADAPTER DTOs | REBUILD-FINAL | Duplicated client transport types for inherited worker boundary; final product should derive/share canonical contracts deliberately. |
...
| `project-docs/architecture/semantic-plan-lifecycle.md` | REFERENCE-DOC / CANONICAL-CONCEPT | EXTRACT-CONCEPT | Strong source for final lifecycle invariants, while transport/compatibility references are lab-specific. |
