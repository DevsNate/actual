...
WHERE e.entity_kind = 'be_master_categories'
...
WHERE e.entity_kind = 'be_subcategories'
  AND e.is_tombstone = false
...
INSERT INTO semantic_monthly_category_budgets
  (budget_id, monthly_category_budget_id, category_id, month,
   budgeted_milliunits, goal_snoozed_at, note, overspending_handling,
   is_tombstone)
SELECT e.budget_id,
       e.entity_id,
       e.payload->>'subCategoryId',
       (e.payload->>'month')::date,
       (e.payload->>'budgeted')::bigint,
...
WHERE e.entity_kind = 'be_monthly_subcategory_budgets'
  AND e.is_tombstone = false
